# JUCE API Research for Fracture — Granular Delay Plugin
...
FractureAudioProcessorEditor::getResource(const juce::String& url)
{
...
FractureAudioProcessor::createParameterLayout()
{
...
```cpp
// Example skew values for Fracture parameters:
...
void FractureAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer,
...
**For Fracture:** Use `Lagrange3rd` for highest quality pitch-shifting grains.
...
**Usage in Fracture (Feedback Filtering):**
...
void FractureAudioProcessor::updateFeedbackFilter()
{
...
**Usage Pattern (if Fracture has processing latency):**
...
**Note:** If Fracture has zero latency (pure delay line processing), DryWetMixer is optional — can manually mix:
...
```cpp
void FractureAudioProcessor::processMIDI(juce::MidiBuffer& midiMessages)
{
...
void FractureAudioProcessor::updateTempoSync()
{
...
float FractureAudioProcessor::tempoSyncToMs(int noteValue)
{
...
target_link_libraries(Fracture
    PRIVATE
        Fracture_UIResources          # Link binary data
...
juce_generate_juce_header(Fracture)
...
# C++ standard
target_compile_features(Fracture PRIVATE cxx_std_17)
```
...
**Next Steps:** Use this research to implement Fracture plugin following the architecture plan.
